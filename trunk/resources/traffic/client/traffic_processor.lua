...
function pedFormQueue ( ped, node, prev )
	...
	for i, v in ipairs ( _peds[ped].debug ) do
		destroyElement ( v )
	end
	_peds[ped].debug = {}
	...
	for i, v in ipairs ( _peds[ped].nodes ) do
		if DEBUG then
			table.insert ( _peds[ped].debug, createMarker ( v.x, v.y, v.z, "corona", 1, 255, 255, 0, 255 ) )
		end
		local node = v
		local next = _peds[ped].nodes[i+1] or v
		local orot = (360 - math.deg(math.atan2((next.x - node.x), (next.y - node.y)))) % 360
		local ox, oy = calcNodeLaneOffset(next, orot)
...
function pedGetNextNode ( ped, node, prev )
	...
	local rand = arrayGetRandom ( neighbours, prev.id )
	return getNode ( rand )
end
