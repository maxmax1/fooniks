...
function LoadSkillsForPlayer( thePlayer )
	...
	if( result ~= false ) then
		...
		if( mysql_num_rows( result ) > 0 ) then
			...
			-- TODO: Fetch all fields, if is a skill field set skill. :)
			-- pSkillLevel
...
function SaveSkillsForPlayer( thePlayer )
	...
	local query = call( getResourceFromName ( "phoenix-Base" ), "MysqlUpdatebuild", "ph_Skills");
	local added = false;	
	
	for k,v in ipairs( skills ) do
	
		query = call( getResourceFromName( "phoenix-Base" ), "MysqlSetField", query, k, pXp[k][thePlayer] );
  		added = true;
	...
	if( added ) then
		...
		-- Finish query.
		query = call( getResourceFromName ( "phoenix-Base" ), "UpdateFinish", query, "cid", charId);
